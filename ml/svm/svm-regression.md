...
在SVM分类模型中，我们的KKT条件的对偶互补条件为： $$\alpha_{i}^{*}(y_i(w \bullet \phi(x_i) + b) - 1) = 0$$，而在回归模型中，我们的对偶互补条件类似如下：
$$
...
根据上面w的计算式$$w = \sum\limits_{i=1}^{m}(\alpha_i^{\land} - \alpha_i^{\lor})\phi(x_i)$$，我们发现此时$$\beta_i = 0$$,也就是说w不受这些在误差范围内的点的影响。对于在边界上或者在边界外的点，$$\alpha_i^{\lor} \neq 0, \alpha_i^{\land} \neq 0$$，此时$$\beta_i \neq 0$$。
