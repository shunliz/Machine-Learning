...
假设样本从隐藏状态$$q_i$$转移到$$q_j$$的频率计数是$$A_{ij}$$,那么状态转移矩阵求得为：$$A = \Big[a_{ij}\Big], \;$$其中$$a_{ij} = \frac{A_{ij}}{\sum\limits_{s=1}^{N}A_{is}}$$

假设样本隐藏状态为$$q_j$$且观测状态为$$v_k$$的频率计数是$$B_{jk}$$,那么观测状态概率矩阵为：$$B= \Big[b_{j}(k)\Big], \;$$其中$$b_{j}(k) = \frac{B_{jk}}{\sum\limits_{s=1}^{M}B_{js}}$$
