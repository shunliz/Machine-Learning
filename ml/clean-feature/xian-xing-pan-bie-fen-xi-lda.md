...
其中x为非零向量，而A,B为$$n \times n$$的Hermitan矩阵。B为正定矩阵。它的最大值和最小值是什么呢？其实我们只要通过将其通过标准化就可以转化为瑞利商的格式。我们令$$x'=B^{-1/2}x$$,则分母转化为：$$x^HBx = x'^H(B^{-1/2})^HBB^{-1/2}x' = x'^HB^{-1/2}BB^{-1/2}x' = x'^Hx'$$

而分子转化为：$$x^HAx =  x'^HB^{-1/2}AB^{-1/2}x'$$

此时我们的R\(A,B,x\)转化为$$R(A,B,x'):R(A,B,x') = \frac{x'^HB^{-1/2}AB^{-1/2}x'}{x'^Hx'}$$
...
这样我们的优化目标重写为：$$\underbrace{arg\;max}_w\;\;J(w) = \frac{w^TS_bw}{w^TS_ww}$$
...
常见的一个LDA多类优化目标函数定义为：$$\underbrace{arg\;max}_W\;\;J(W) = \frac{\prod\limits_{diag}W^TS_bW}{\prod\limits_{diag}W^TS_wW}$$
