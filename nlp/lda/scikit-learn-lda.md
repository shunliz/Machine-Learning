...
我们接着导入停用词表，这里的代码和[中文文本挖掘预处理流程总结](/nlp/preprocessing.md)中一样，如果大家没有1208个的中文停用词表，可以到之前的这篇文章的链接里去下载。文件导入停用词表
...
在实际的应用中，我们需要对K,$$\alpha$$,$$\eta$$进行调参。如果是"online"算法，则可能需要对"online"算法的一些参数做调整。这里只是给出了LDA主题模型从原始文档到实际LDA处理的过程
