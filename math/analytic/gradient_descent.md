...
1. 先决条件： 确认优化模型的假设函数和损失函数。  
   ...
   $$
   , 其中$$\theta_i (i = 0,1,2... n)$$为模型参数，$$x_i(i = 0,1,2... n)$$为每个样本的n个特征值。这个表示可以简化，我们增加一个特征$$x_0=1$$ ，这样$$h_\theta(x_0,x_1...x_n)=\sum_{i=0}^{n}\theta_ix_i$$。同样是线性回归，对应于上面的假设函数，损失函数为：
...
下面用线性回归的例子来具体描述梯度下降。假设我们的样本是$$(x^{(0)}_1,x^{(0)}_2,...x^{(0)}_n,y_0),(x^{(1)}_1,x^{(1)}_2,...x^{(1)}_n,y_1),...(x^{(m)}_1,x^{(m)}_2,...x^{(m)}_n,y_n)$$,损失函数如前面先决条件所述：
$$
...
$$
\theta_i=\theta_i-\alpha\sum_{j=t}^{t+x-1}(h_\theta(x^j_0,x^j_1,...x^j_n)-y_j)x^j_i
$$
...
$$
\theta_i=\theta_i-\alpha(h_\theta(x^j_0,x^j_1,...x^j_n)-y_j)x^j_i
$$
